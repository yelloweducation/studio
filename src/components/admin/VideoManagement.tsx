...
import { PlusCircle, Edit3, Trash2, Video as VideoIcon } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
const VideoForm = ({ 
  ...
  const [thumbnailUrl, setThumbnailUrl] = useState(video?.thumbnailUrl || 'https://placehold.co/300x500.png');
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const videoData: Video = { 
      ...
      thumbnailUrl,
      dataAiHint,
      videoUrl: video?.videoUrl || '', // Keep existing or empty
    };
    onSubmit(videoData);
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <ScrollArea className="h-[60vh] sm:h-[70vh] pr-4">
        <div className="space-y-4">
          ...
          <div>
            <label htmlFor="thumbnailUrl" className="block text-sm font-medium text-foreground mb-1">Thumbnail URL</label>
            <Input id="thumbnailUrl" value={thumbnailUrl} onChange={e => setThumbnailUrl(e.target.value)} />
            {thumbnailUrl && (
              <div className="mt-2 relative w-32 h-48 border rounded overflow-hidden">
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Add, edit, or delete videos for the platform.</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <Dialog open={isFormOpen} onOpenChange={(isOpen) => { if(!isOpen) closeForm(); else setIsFormOpen(true);}}>
            ...
            <DialogContent className="sm:max-w-xl"> {/* Adjusted width */}
              <DialogHeader className="pb-4">
                ...
                <DialogDescription>
                  {editingVideo ? 'Modify the details of the existing video.' : 'Fill in the details for the new video.'}
                </DialogDescription>
              </DialogHeader>
              <VideoForm 
                video={editingVideo} 
          ...
          <ul className="space-y-4">
            {videos.map(video => (
              <li key={video.id} className="p-3 sm:p-4 border rounded-lg bg-card flex flex-col gap-3 sm:flex-row sm:items-center shadow-sm hover:shadow-md transition-shadow">
                {video.thumbnailUrl && (
                  <div className="relative w-20 h-28 sm:w-24 sm:h-36 border rounded overflow-hidden shrink-0">
