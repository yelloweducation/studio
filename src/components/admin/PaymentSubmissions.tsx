...
import { paymentSubmissions_DEPRECATED_USE_FIRESTORE as initialSubmissions, users as mockUsers, type PaymentSubmission, type User, type Course, type PaymentSubmissionStatus } from '@/data/mockData'; // Keep mockUsers for now if user data isn't fully in Firestore yet
...
import { getPaymentSubmissionsFromFirestore, updatePaymentSubmissionInFirestore, getCourseByIdFromFirestore, getCoursesFromFirestore } from '@/lib/firestoreUtils'; // Adjust if user fetching changes
import { getAllUsersFromFirestore } from '@/lib/authUtils'; // Assuming user data is also in Firestore

export default function PaymentSubmissions() {
  const [submissions, setSubmissions] = useState<PaymentSubmission[]>([]);
  const [allUsers, setAllUsers] = useState<User[]>([]); // Fetch from Firestore
  const [allCourses, setAllCourses] = useState<Course[]>([]); // Fetch from Firestore
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        const [submissionsFromDb, usersFromDb, coursesFromDb] = await Promise.all([
          getPaymentSubmissionsFromFirestore(),
          getAllUsersFromFirestore(), // Make sure this fetches all users needed
          getCoursesFromFirestore() // Fetch all courses
        ]);
  ...
  const handleUpdateStatus = async (submissionId: string, newStatus: PaymentSubmissionStatus, notes?: string) => {
    setIsUpdating(true);
    try {
      await updatePaymentSubmissionInFirestore(submissionId, { status: newStatus, adminNotes: notes, reviewedAt: new Date().toISOString() });
    ...
    return (
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Loading payment submissions from Firestore...</CardDescription>
        </CardHeader>
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Manage and review manual payment submissions from users (data from Firestore).</CardDescription>
      </CardHeader>
      <CardContent>
        {submissions.length > 0 ? (
          ...
          <p className="text-center text-muted-foreground py-6">No payment submissions found in Firestore.</p>
        )}
      </CardContent>
